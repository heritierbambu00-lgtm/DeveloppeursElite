    ...
    <section id="services" className="py-24 lg:py-32 bg-white dark:bg-luma-dark border-y border-line dark:border-white/5 transition-colors duration-500">
      <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 xl:px-10 2xl:max-w-[90rem]">
        ...
        <div className="border-t border-line dark:border-white/10">
          {services.map((s, idx) => (
            <div key={idx} className="rv border-b border-line dark:border-white/10">
              <button
              ...
              >
                <span className="font-display font-semibold text-smoke dark:text-white/20 group-hover:text-clay transition-colors">
                  {s.num_str}
                </span>
                <span className="font-display font-bold text-xl sm:text-2xl lg:text-[1.65rem] tracking-tight text-ink dark:text-white group-hover:text-clay transition-colors uppercase italic">
                  {s.title}
                </span>
                <span
                  className={`justify-self-end w-9 h-9 rounded-full border border-line dark:border-white/10 grid place-items-center transition-all duration-500 ${
                    openAcc === idx ? 'rotate-45 bg-ink dark:bg-clay border-ink dark:border-clay text-paper' : 'dark:text-white/40'
                  }`}
                >
                ...
                <div>
                  <div className="grid md:grid-cols-12 gap-8 pb-9 md:pl-[4.5rem]">
                    <div className="md:col-span-7 min-w-0">
                      <p className="text-[15px] leading-relaxed text-ink/75 dark:text-white/60 font-medium">{s.description}</p>
                      <div className="mt-5 flex flex-wrap gap-2">
                        {s.tags.map((t, i) => (
                          <span key={i} className="inline-flex items-center gap-1.5 text-[11.5px] font-semibold px-3 py-1.5 bg-paper dark:bg-white/5 border border-line dark:border-white/10 rounded-full dark:text-white/60">
