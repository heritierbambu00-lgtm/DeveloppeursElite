  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statsCards.map((s, i) => (
          <div key={i} className="bg-luma-card backdrop-blur-xl border border-white/5 p-8 rounded-[2rem] relative overflow-hidden group hover:border-white/10 transition-all">
             <div className="flex justify-between items-start relative z-10">
                <div className={`w-12 h-12 rounded-2xl bg-${s.color}/10 flex items-center justify-center text-${s.color} border border-${s.color}/20 shadow-inner`}>
                   <i className={`fa-solid ${s.icon} text-xl`}></i>
                </div>
                <span className="text-[10px] font-black text-moss bg-moss/10 px-3 py-1 rounded-full uppercase tracking-widest">{s.trend}</span>
             </div>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
         {/* Main Promo Area */}
         <div className="lg:col-span-12 bg-luma-card backdrop-blur-xl border border-white/5 rounded-[3rem] overflow-hidden flex flex-col md:flex-row relative group">
            <div className="p-12 lg:p-16 flex-1 relative z-10 flex flex-col justify-center">
               <span className="text-[11px] font-black uppercase tracking-[0.3em] text-luma-purple mb-6 block">Develite Ecosystem</span>
               <h2 className="text-6xl font-display font-black leading-none mb-8 tracking-tighter">Elevate Your <br/> <span className="text-luma-purple">Impact.</span></h2>
               <p className="text-white/40 text-lg leading-relaxed mb-10 max-w-md font-medium">
                  Centralisez vos opérations technologiques et pilotez votre croissance avec DEVELITE AI.
               </p>
               <div className="flex flex-wrap gap-5">
                  <Link to="/admin/projects" className="bg-white text-luma-dark px-10 py-4 rounded-2xl text-sm font-black hover:bg-luma-purple hover:text-white transition-all shadow-xl shadow-white/5 active:scale-95">View Projects →</Link>
                  <Link to="/admin/users" className="bg-white/5 text-white px-10 py-4 rounded-2xl text-sm font-black border border-white/10 hover:bg-white/10 transition-all active:scale-95">Manage Team</Link>
               </div>
            </div>
            <div className="flex-1 min-h-[400px] relative overflow-hidden">
               ...
               <div className="absolute bottom-1/4 right-1/2 w-40 h-40 bg-luma-blue/10 rounded-full blur-3xl animate-pulse [animation-delay:1s]"></div>
            </div>
         </div>
      </div>

      {/* Dynamic Activity / Recommended Grid */}
      <div className="grid lg:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white/[0.03] border border-white/5 p-6 rounded-[2rem] flex items-center gap-5 hover:bg-white/[0.05] transition-all cursor-pointer group">
              <div className={`w-12 h-12 rounded-xl bg-${card.color}/10 flex items-center justify-center text-${card.color} border border-${card.color}/10`}>
                 <i className={`fa-solid ${card.icon}`}></i>
              </div>
              <div>
                <p className="text-white font-bold text-sm tracking-tight">{card.label}</p>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/30 group-hover:text-luma-purple transition-colors">{card.status}</p>
              </div>
            </div>
          ))}
      </div>
